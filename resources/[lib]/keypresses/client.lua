RegisterKeyMapping('Menu',          'Open & Close Menu',    'keyboard', 'M')
RegisterKeyMapping('RepairVehicle', 'Repair Vehicle',       'keyboard', 'K')
RegisterKeyMapping('ToggleEngine',  'Toggle Engine State',  'keyboard', 'N')
RegisterKeyMapping('Sit',           'Sit',                  'keyboard', 'E')
RegisterKeyMapping('LeftTurn',      'Indicator Left Turn',  'keyboard', 'LEFT')
RegisterKeyMapping('RightTurn',     'Indicator Right Turn', 'keyboard', 'RIGHT')
RegisterKeyMapping('Hazard',        'Indicator Hazard',     'keyboard', 'DOWN')

CreateThread(function()
    local NKey = 249 -- N key
    local KKey = 311 -- K key

    while true do
        Wait(0)

        -- Repair current vehicle
        if IsControlJustReleased(1, KKey) then
        ...
        if IsControlJustReleased(1, NKey) then
            local playerPed = PlayerPedId()
...
-- Indicators (Left/Right/Both/Off)

local indicator = "Off"

RegisterNetEvent('pv:syncIndicator', function (playerId, IStatus)
...
RegisterNetEvent('pv:setHazards', function(hazardsDeactivate)
	...
	while true do
		...
		for playerId = 0, 2 do
			if NetworkIsPlayerActive(playerId) then
				local networkPed = GetPlayerPed(GetPlayerFromServerId(playerId))
				local networkPedVeh = GetVehiclePedIsIn(networkPed, false)
				
				if networkPedVeh ~= nil and networkPedVeh ~= false and GetPlayerFromServerId(playerId) ~= PlayerId() and GetPedInVehicleSeat(networkPedVeh, -1) == networkPed and IsVehicleEngineOn(networkPedVeh) then
