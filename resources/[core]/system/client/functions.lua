function System.IsPlayerLoaded()
    return System.PlayerLoaded
end

function System.GetPlayerData()
    return System.PlayerData
end

function System.SetPlayerData(key, val)
...
function System.SpawnActiveVehicle(enterVehicle)
    if System.PlayerData.vehicles and #System.PlayerData.vehicles > 0 then
        ...
        if not vehicle then
            LocalPlayer.state:set('activeVehicle', nil, true)
            LocalPlayer.state:set('activeVehicleNet', nil, true)
            return
        end
...
RegisterNetEvent('system:vehicleCreated', function(netId, vehicle, enterVehicle)
    ...
    local newVehicle = NetworkGetEntityFromNetworkId(netId)

    Entity(newVehicle).state:set('mileage', vehicle.mileage, true)
    ...
    SetVehicleNumberPlateText(newVehicle, vehicle.plate)
    SetRadioToStationName(newVehicle, 'OFF')
    SetEntityAsMissionEntity(newVehicle, true, true)
    FreezeEntityPosition(newVehicle, false)
    NetworkRegisterEntityAsNetworked(newVehicle)
    SetNetworkIdCanMigrate(netId, false)

    System.SetVehicleProperties(newVehicle, vehicle.props)

    TriggerServerEvent('system:updateVehicleState', vehicle.plate, netId)
    ...
    Citizen.CreateThread(function()
        ...
        blip = AddBlipForEntity(newVehicle)
        ...
        while DoesEntityExist(newVehicle) and IsVehicleActive(newVehicle) do
            if GetVehiclePedIsIn(PlayerPedId(), false) == newVehicle then
                if not hasBeenRemoved then
                    RemoveBlip(blip)
                    hasBeenRemoved = true
                end
            else
...
function IsVehicleActive(vehicle)
    ...
    return LocalPlayer.state.activeVehicle == plate
end

function System.GetVehicleProperties(vehicle)
...
function System.SetVehicleProperties(vehicle, props)
    if not DoesEntityExist(vehicle) then
        return
    end
